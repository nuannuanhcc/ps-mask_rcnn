...
def do_prw_evaluation_reid(dataset, predictions, output_folder, logger):
...
def calc_reid_prw_topk(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, topk, det_thresh=0.5, iou_thresh=0.5, gallery_size=100):
    ...
    assert len(query_img_list) == 2057, "Length of query_img lists need to be 2900, but it is {}.".format(len(query_img_list))
    assert len(query_boxlists) == 2057, "Length of query_boxlists lists need to be 2900, but it is {}.".format(len(query_boxlists))
    ...
    ############################# path #############################
    annotation_dir = './data/prw/SIPN_annotation/'
...
def eval_reid_prw(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, iou_thresh=0.5, use_07_metric=False, gallery_size=100):
    ...
    topk = [1, 3, 5, 10]
    #CMC = []
    real_result, ideal_result = calc_reid_prw_topk(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, topk, det_thresh=0.5, iou_thresh=0.5, gallery_size=gallery_size)
