...
def do_sysu_evaluation_reid(dataset, predictions, output_folder, logger):
    ...
    for image_id, prediction in enumerate(tqdm(predictions_test)):
        ...
        img_info = dataset_test.get_img_info(image_id)
        if len(prediction) == 0:
            continue
        # reid
        img_name = dataset_test.ids[image_id]
        ...
        gt_boxlist = dataset_test.get_groundtruth(image_id)
        gt_boxlists.append(gt_boxlist)

    gallery_size = dataset_query.gallery_size
    print('###################   gallery_size = {}   ##################'.format(gallery_size))
    result = eval_reid_sysu(
...
def calc_reid_sysu_prec_rec(gt_boxlists, pred_boxlists, iou_thresh=0.5):
...
def calc_reid_sysu_ap(prec, rec, use_07_metric=False):
...
def calc_reid_sysu_topk(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, topk, det_thresh=0.5, iou_thresh=0.5, gallery_size=100):
    ...
    logger = logging.getLogger("sysu_reid_benchmark.evaluation\n")
    ...
    gallery_feat = [pred_boxlist.get_field('reid_feat').numpy() for pred_boxlist in pred_boxlists]
    probe_feat = [query_boxlist.get_field('reid_feat').numpy() for query_boxlist in query_boxlists]
    ...
    for i in range(len(probe_feat)):
        #print ('calc_reid_sysu_topk i = {} ...'.format(i))
        #embed()  ###
        ...
        for g_i in range(1, gallery_size + 1):
            #print ('calc_reid_sysu_topk g_i = {} ...'.format(g_i))
            #embed()  ###
...
def eval_reid_sysu(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, iou_thresh=0.5, use_07_metric=False, gallery_size=100):
    ...
    prec, rec = calc_reid_sysu_prec_rec(pred_boxlists=pred_boxlists, gt_boxlists=gt_boxlists, iou_thresh=iou_thresh)
    ...
    ap = calc_reid_sysu_ap(prec, rec, use_07_metric=use_07_metric)
    ...
    topk = [1, 3, 5, 10]
    #CMC = []
    real_result, ideal_result = calc_reid_sysu_topk(test_img_list, query_img_list, query_boxlists, pred_boxlists, gt_boxlists, topk, det_thresh=0.5, iou_thresh=0.5, gallery_size=gallery_size)
